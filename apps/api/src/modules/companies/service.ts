...
export class CompaniesService {
  ...
  async list(tenantId: string, query: ListCompaniesQuery) {
    ...
    const [items, countResult] = await Promise.all([
      this.db.select().from(companies).where(and(...conditions)).orderBy(desc(companies.createdAt)).limit(limit).offset(offset),
      this.db.select({ count: sql<number>`count(*)` }).from(companies).where(and(...conditions)),
    ]);
  ...
  async create(tenantId: string, data: CreateCompanyInput) {
    ...
    await this.db.insert(nsuControl).values([
  ...
  async update(tenantId: string, companyId: string, data: UpdateCompanyInput) {
    ...
    if (data.cnpj && data.cnpj !== existing.cnpj) {
      const cnpjExists = await this.db.query.companies.findFirst({
...
export const companiesService = container.resolve(CompaniesService);
